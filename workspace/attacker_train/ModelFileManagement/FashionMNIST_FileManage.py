...
class FashionMNISTFileManagement(FileManagement):
    ...
    def save_substitute_model(self):
        ...
        if self.Fractal == True:
            save_name = f"size_{self.model_size}_query_{self.access_times}_{self.Fracdata}_FashionMNIST"
             # Load the image
            ...
            save_name = f"size_{self.model_size}_query_{self.access_times}_Batch_{self.batch_size}_FashionMNIST"
