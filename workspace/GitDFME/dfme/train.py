...
def train(args, teacher, student, generator, device, optimizer, epoch):
    ...
    for i in range(args.epoch_itrs):
        ...
        for _ in range(args.g_iter):
            ...
            #修正GenA param = z, pre_x=args.approx_grad
            ...
            approx_grad_wrt_x, loss_G = estimate_gradient_objective(args, teacher, student, fake, 
                                                epsilon = args.grad_epsilon, m = args.grad_m, num_classes=args.classes, 
...
def test(args, student = None, generator = None, device = "cuda", test_loader = None, epoch=0, best_acc=0):
    global file
    student.eval()
    generator.eval()
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            ...
            target = target.squeeze()  # ターゲットテンソルを1次元に変換
    ...
    print("現ラウンドの攻撃精度",accuracy)
    print("ベストの攻撃精度",best_acc*100)
    if best_acc*100 > accuracy:
...
def main():
    ...
    # 変更箇所
    parser.add_argument('--dataset', type=str, default='MNIST', choices=['MNIST','FashionMNIST','MedMNIST'], help='dataset name (default: MNIST)')
    ...
    # 変更箇所
    parser.add_argument('--base_target_load_path', type=str, default="/workspace/cloud_model/checkpoints_cloud/")
    parser.add_argument('--target_load_name', type=str, default=None)
    parser.add_argument('--student_load_path', type=str, default=None)
    parser.add_argument('--G_net_load_path', type=float, default=0)
    parser.add_argument('--G_net_data', type=str, default='MNIST', choices=['MNIST','FashionMNIST','Fractal','MedMNIST','LDFractal','OCTMNIST', 'pneumoniamnist', 'tissuemnist'])
    parser.add_argument('--medflag', type=str, default='organamnist')
    ...
    if args.dataset == "MedMNIST":
        pre_pre_path = os.path.join(args.base_target_load_path, args.dataset)
        pre_path = os.path.join(pre_pre_path, args.medflag)
    else:
        pre_path = os.path.join(args.base_target_load_path, args.dataset)
    args.target_load_path = os.path.join(pre_path, args.target_load_name)
    print("Target_model_path : ",args.target_load_path)
    # target_net = networks_target.TargetNetwork_MNIST()
    teacher = networks_target.AttackerNetworkSmall_MNIST(args.classes)
    teacher.load_state_dict(torch.load(args.target_load_path)['model_state_dict'])
    ...
    # 変更箇所
    student = networks_attacker.AttackerNetworks_MNIST_M(args.classes)
    # 修正GeneratorA param = nz=args.nz, nc=1, img_size=28, activation=args.G_activation
    generator = network.gan.GeneratorA(nz=args.nz, nc=1, img_size=28, activation=args.G_activation, final_bn=False)
    ...
    args.student_path = "NOS_load"
    args.G_net_path = "NOG_load/"

    if args.G_net_load_path != 0 :
        args.G_net_path = "G_load/"
        savepath = os.path.join('/workspace/network_Gan/', args.G_net_data, 'net_param.py')
        generator.load_state_dict(torch.load(savepath))
        print("Load path", savepath)
        print("Suceess Loading Generator_net")
        

        
    if args.student_load_path :
        args.student_path = "S_load"
    ...
    # Connecting New save path
    args.log_dir = os.path.join(args.log_dir, args.dataset, args.G_net_path, args.student_path, args.G_net_data, str(args.lr_G), str(args.g_iter), str(args.lr_S), str(args.d_iter), str(args.scale))

    print("save_path", args.log_dir)
    # フォルダを作成
    ...
    for epoch in range(1, number_epochs + 1):
        ...
        # Test
        acc = test(args, student=student, generator=generator, device = device, test_loader = test_loader, epoch=epoch, best_acc=best_acc )
        acc_list.append(acc)
        if acc>=best_acc:
            best_acc = acc
            name = 'resnet34_8x'
        ...
        # else:
        #     print("Sucsess load")
        #     loadpath = os.path.join(args.log_dir, "checkpoint/student.pt")
        #     checkpoint = torch.load(loadpath)
        #     student.load_state_dict(checkpoint)
        #     loadpaths = os.path.join(args.log_dir, "checkpoint/generator.pt")
        #     checkpoints = torch.load(loadpaths)
        #     generator.load_state_dict(checkpoints)
